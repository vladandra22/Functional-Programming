...
(define (tail-get-woman wpref L)
  (if(null? wpref)
     L
     (tail-get-woman (cdr wpref) (append L (list (caar wpref))))
     )
  )

  
(define (get-women wpref)
  (tail-get-woman wpref '())
  )
...
(define (preferable? pref-list x y)
  (and (not (null? pref-list))
       (or (equal? x (car pref-list))
           (and (not (equal? y (car pref-list)))
...
(define (get-partner engagements person)
  (cond
    ((null? engagements) #f)
    ((equal? (caar engagements) person) (cdar engagements))
