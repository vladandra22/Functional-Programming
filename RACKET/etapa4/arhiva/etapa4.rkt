          ...
          (if (null? pref-list)
            ...
            ;altfel, cautam partener
              (letrec ((current-pref (car pref-list))
                       ...
                       (other-partner (get-partner engagements current-pref)))
               (cond
                  ; daca partenerul preferat nu e in aceeasi camera, trecem la urmatorul posibil partener pentru person
                  ((not (member current-pref (get-couple-members engagements))) (verify-partner (cdr pref-list)))
                  ...
                  ;daca desi are partener, dar nu il prefera pe partenerul curent,
                  ; actualizam engagements-urile si cautam un match pentru noul ex, care acum devine singur
                  ((preferable? partner-pref-list person other-partner) (match other-partner (update-engagements engagements current-pref person) pref1 pref2 queue))
