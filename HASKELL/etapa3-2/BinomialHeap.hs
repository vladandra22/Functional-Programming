...
isolate :: a -> [a] -> [(a, [a])]
isolate _ [] = []
isolate placeholder list = func 0 list
  where
    func 0 [] = []
    func index (x:[]) =  [(x, init list)] -- daca e ultimul element din lista, iau lista fara placeholder si fara ultimul element
    -- pentru restul elementelor, construiesc lista si apelez recursiv pentru restul listei
    func index (x:xs) = (x, newList) : func (index + 1) xs
      -- luam primele index elemente ale listei, adaugam placeholderul si dupa luam restul de elemente dupa index+1.
        where newList = take index list ++ [placeholder] ++ drop (index + 1) list
...
removeMin (BinomialHeap _ []) = emptyHeap
...
-- functie implementata pe modelul functiei printLevel din laborator
printLevel :: (Show p, Show k) => Char -> Int -> BinomialTree p k -> [Char]
printLevel _ _ EmptyTree = ""
printLevel space level (Node p k c) =
  replicate (level * 2) space ++ show p ++ " (" ++ show k ++ ")"
    ++ (if null c then "" else "\n") ++ intercalate "\n" (map (printLevel space (level + 1)) c)
-- mapam functia printLevel fiecarui copil al arborelui, intercaland listele rezultate
-- intr-o singura lista
...
-- deoarece avem instante ale lui show pentru copacii heap-ului, 
-- putem mapa functia show pentru fiecare copac si concatenam listele si adaugam
-- "/n" pentru a separa arborii. rezultatul va contine cate un BinomialTree
-- pe fiecare linie
instance (Show p, Show k) => Show (BinomialHeap p k) where
  show heap@(BinomialHeap p k) = intercalate "\n" (map show k)
{-
    *** TODO ***
...
instance Functor (BinomialTree p) where
    ...
    -- aplicam functia f pe cheile arborilor si o apelam recursiv pentru restul copiilor
    -- rezultatul este un arbore cu aceeasi structura, dar pe care putem aplica o proprietate (cum e toUpper)
    fmap f tree@(Node p k c) = (Node p (f k) (map (fmap f) c))
...
instance Functor (BinomialHeap p) where
    ...
    -- avand aceasta instanta deja pentru BinomialTree, putem apela recursiv
    -- pentru copiii lui BinomialHeap recursiv aceeasi functie fmap
    -- rezultatul va fi acelasi, un heap cu aceeasi structura, unde elementele
    -- au o anumita proprietate (ex: toUpper)
    fmap f heap@(BinomialHeap p k) = (BinomialHeap p (map (fmap f) k))
